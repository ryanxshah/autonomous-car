...
class TransformerPlanner(nn.Module):
    def __init__(
        ...
        self.latent_transformer = nn.ModuleList(
            [nn.TransformerEncoderLayer(d_model=d_model, nhead=4, batch_first=True) for _ in range(2)]
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        latent_array = self.query_embed.weight.unsqueeze(0).expand(inputs.size(0), -1, -1) # (b, n_waypoints, d_model)
